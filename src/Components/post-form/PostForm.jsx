...
const PostForm = ({post}) => {
    ...
    const submit = async(data)=>{
        if(post){
            const file= data?.image[0] ? await uploadservices.uploadFile(data.image[0]):null;

            if (file){
                uploadservices.deleteFile(post.feturedImage)
            }

            const dbPost =await configService.updatePost(post.$id,{...data,feturedImage:file?file.$id:undefined});
            if (dbPost) {
                navigate(`/post/${dbPost.$id}`);
            }
        }  else{
            debugger
            const file = await uploadservices.uploadFile(data.image[0]);
            if (file){
